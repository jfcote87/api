...
var (
	...
	googleAPIPkg = flag.String("googleapi_pkg", "github.com/jfcote87/api", "Go package path of the 'googleapi' support package.")
...
func (meth *Method) generateCode() {
	...
	if meth.supportsMediaDownload() {
		...
		pn("\nreturn res, c.caller_.Do(%s, c.s.client, call)", contextName)
		pn("}")
	}
	...
	if returnsValue {
		pn("\nreturn returnValue, c.caller_.Do(%s, c.s.client, call)", contextName)
	} else {
		pn("\nreturn c.caller_.Do(%s, c.s.client, call)", contextName)
	}
